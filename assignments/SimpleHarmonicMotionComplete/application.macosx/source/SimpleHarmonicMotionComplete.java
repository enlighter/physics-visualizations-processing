...
public class SimpleHarmonicMotionComplete extends PApplet {
...
public void draw()
{
  ...
  line(0, 1.5f*topPart, width, 1.5f*topPart);
  
  //draw the axis for the acceleration curve
  line(0, 2.5f*topPart,width, 2.5f*topPart);
  ...
  text("k", width/8 + 20, topPart/12 + 2.5f*SpringUnitWidth);
  ...
  stroke(160);
  noFill();
  ellipse(width/8, 1.5f*topPart, radius, radius);
  line(width/8, (1.5f*topPart)-radius/2, width/8, (1.5f*topPart)+radius/2);
  ...
  stroke(0);
  drawArrow( (width/8)-(radius/2), 1.5f*topPart, radius/2, -90);
  line( (width/8)-(radius/2)-8, 1.5f*topPart - radius/2, (width/8)-(radius/2)+8, 1.5f*topPart - radius/2);
  
  //denote the maximum value of acceleration
  stroke(0);
  drawArrow( (width/8)-(radius/2), 2.5f*topPart, radius/2, -90);
  line( (width/8)-(radius/2)-8, 2.5f*topPart - radius/2, (width/8)-(radius/2)+8, 2.5f*topPart - radius/2);
  ...
  for (int i = 0; i< width; i++){
    pxS = width/8 + cos(radians(angleS))*(radius/2);
    pyS = 2.5f*topPart - sin(radians(angleS))*(radius/2);
    point(width/8+radius/2+i, pyS);
    angleS -= frequencyS;
  }
  ...
  drawArrow((float)width/8, 1.5f*topPart, velVL, (float)-90);
  
  //draw the acceleration vector
  drawArrow((float)width/8, 2.5f*topPart, accVL, (float)-90);
  ...
  line(width/8, (1.5f*topPart)-velVL, width/8+radius/2+px, (1.5f*topPart)-velVL);

  //draw dynamic line connecting top of velocity vector 
  //with the velocity curve
  line(width/8, (2.5f*topPart)-accVL, width/8+radius/2+px, (2.5f*topPart)-accVL);
...
public void drawArrow(float cx, float cy, float len, float angle){
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "SimpleHarmonicMotionComplete" });
  }
}
