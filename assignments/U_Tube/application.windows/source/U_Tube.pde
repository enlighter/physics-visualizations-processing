...
void setup()
{
  ...
  columnHeight = 180;
  angle=0;
  frequency= 4.5 - 2*(sqrt(PI/2*.8+1.8)/sqrt(PI/2*.8+1.4));
  println(frequency);
  ...
  Length.setLimits(PI/2*.8+1.8, PI/2*.8+1.4, PI/2*.8+2.2);
  ...
  Displacement.setLimits(0.4, 0.0, 0.4);
...
void draw()
{
  ...
  noStroke();
  fill(0);
  rect(400, 400-columnHeight-maxVariation-20, ( float )360/2.35 + 61, 2*maxVariation+40);
  noFill();
  ...
  text("L = π/2*R + "+columnHeight/100+" = "+(PI/2*0.8+columnHeight/100)+" cm",100,550);
...
void handleButtonEvents(GButton button)
{
  switch(button.eventType)
  {
      case GButton.CLICKED:
        ...
        frequency = 4.5 - 2*(sqrt(Length.getValuef())/sqrt(PI/2*.8+1.4));
        columnHeight = (Length.getValuef()-(PI/2*.8))*100;
